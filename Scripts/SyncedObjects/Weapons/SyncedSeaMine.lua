...
local THROW_OBJECT_FORCE = 5000

local THROW_UP_FORCE = 500

--SYNCEDSEAMINE CLASS START

--SyncedSeaMine
class 'SyncedSeaMine' (ISyncedWeapon)

function SyncedSeaMine:__init() super(20)
	...
	self.proximityRadius = 6
    self.explosionRadius = 23
	self.minExplosionForce = 0
	self.maxExplosionForce = 8000
	self.minUpExplosionForce = 3000
	self.maxUpExplosionForce = 10000

	self.idealScale = WVector3(3, 3, 3)
	...
	self.stateFuncs = { }
	self.stateFuncs[self.SMS_NOT_USED] = { SyncedSeaMine.StateChangeNotUsed, SyncedSeaMine.ProcessStateNotUsed }
	self.stateFuncs[self.SMS_DETACHED] = { SyncedSeaMine.StateChangeDetached, SyncedSeaMine.ProcessStateDetached }
	self.stateFuncs[self.SMS_THROWN] = { SyncedSeaMine.StateChangeThrown, SyncedSeaMine.ProcessStateThrown }
	self.stateFuncs[self.SMS_ARMED] = { SyncedSeaMine.StateChangeArmed, SyncedSeaMine.ProcessStateArmed }
	self.stateFuncs[self.SMS_EXPLODING] = { SyncedSeaMine.StateChangeExploding, SyncedSeaMine.ProcessStateExploding }
	self.stateFuncs[self.SMS_EXPLODED] = { SyncedSeaMine.StateChangeExploded, SyncedSeaMine.ProcessStateExploded }

end


function SyncedSeaMine:BuildInterfaceDefISynced()

	self:AddClassDef("SyncedSeaMine", "ISyncedWeapon", "")

end


function SyncedSeaMine:InitWeapon()
...
function SyncedSeaMine:UnInitWeapon()
...
function SyncedSeaMine:InitGraphical()

	self.graphicalMine = OGREModel()
	local params = Parameters()
	params:AddParameter(Parameter("RenderMeshName", "seamine.mesh"))
...
function SyncedSeaMine:UnInitGraphical()
...
function SyncedSeaMine:InitPhysical(detachBehind)
...
function SyncedSeaMine:UnInitPhysical()
...
function SyncedSeaMine:SetMineState(newState)
...
function SyncedSeaMine:DoesWeaponOwn(ownObjectID)
...
function SyncedSeaMine:NotifyPositionChange(setPos)
...
function SyncedSeaMine:NotifyOrientationChange(setOrien)
...
function SyncedSeaMine:GetPosition()
...
function SyncedSeaMine:GetOrientation()
...
function SyncedSeaMine:GetWeaponActive()
...
function SyncedSeaMine:SetWeaponStateData(stateBuiltTime, setState)
...
function SyncedSeaMine:GetWeaponStateData(returnState)
...
function SyncedSeaMine:UseItemUp(pressed, extraData)

	if IsServer() and self.state == self.SMS_NOT_USED then
		if pressed then
			--If extra data was sent by the client, we can use it here
			--if IsValid(extraData) then
			--	print("EXTRA DATA = " .. tostring(extraData:GetWVector3Data()))
			--end
			self:Throw()
		end
	end
...
function SyncedSeaMine:UseItemDown(pressed, extraData)
...
function SyncedSeaMine:SetWeaponParameter(param)

end


function SyncedSeaMine:EnumerateWeaponParameters(params)

end


--The mine should detach from the kart it is following
function SyncedSeaMine:Detach()
...
function SyncedSeaMine:GetObjectsWithinRadius()
...
function SyncedSeaMine:MineCollision(collisionParams)

	--BRIAN TODO: Collision test code
	--print("SyncedSeaMine:MineCollision() called")
...
function SyncedSeaMine:ProcessSyncedObject(frameTime)
...
function SyncedSeaMine:ProcessStateNotUsed(frameTime)

end


function SyncedSeaMine:ProcessStateDetached(frameTime)
...
function SyncedSeaMine:ProcessStateThrown(frameTime)
...
function SyncedSeaMine:ProcessStateArmed(frameTime)
...
function SyncedSeaMine:ProcessStateExploding(frameTime)
...
function SyncedSeaMine:ProcessStateExploded(frameTime)

end


--State change functions below
function SyncedSeaMine:StateChangeNotUsed(initing)

end


function SyncedSeaMine:StateChangeDetached(initing)
...
function SyncedSeaMine:StateChangeThrown(initing)
...
function SyncedSeaMine:StateChangeArmed(initing)
...
function SyncedSeaMine:StateChangeExploding(initing)
	...
	if initing then
		...
		if IsClient() and IsValid(self.graphicalMine) then
			--Play explode sound
			GetSoundSystem():EmitSound(ASSET_DIR .. "sound/Mine_Explose.wav", self.graphicalMine:GetPosition(), 10, 6, true, SoundSystem.MEDIUM)
...
function SyncedSeaMine:StateChangeExploded(initing)
