...
function ServerSettingsManager:ReadFile()
	...
	if IsValid(self.serverDir) then
		...
		if IsValid(file) then
			...
			for line in file:lines() do
				...
				elseif #line > 0 then
					...
					elseif mode == "#KART_CC" then
					 ...
					 elseif mode == "#START_SPECTATOR" then
						self.startSpectator = line
			...
			print("Start as Spectator:"..self.startSpectator)
			print("Map Cycle:")
...
function ServerSettingsManager:WriteFile()
	...
	if IsValid(self.serverDir) then
		...
		if IsValid(file) then
			...
			file:write("#START_SPECTATOR\n")
			file:write(self.startSpectator.."\n\n")
